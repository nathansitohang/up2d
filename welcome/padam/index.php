...
<html lang="en">
	...
	<body class="fixed-nav sticky-footer bg-dark" id="page-top">
						...
						<?php
							...
							'  
									<tr>
										...
										<td>'.$row["scada"].'</td>  																
										<td><p data-placement="top" data-toggle="tooltip" title="Gangguan"><a data-target="#entryPdm" class="btn btn-danger btn-circle " data-title="Edit" 
										data-toggle="modal" data-id="'.base64_encode(1234*$row["no"]).'" ><i class="fa fa-bolt" style="color:#fff"></i></a></p></td>
										<td><p data-placement="top" data-toggle="tooltip" title="Emergency"><a data-target="#entryEme" class="btn btn-secondary btn-circle " data-title="Eme" 
										data-toggle="modal" data-id="'.base64_encode(4567*$row["no"]).'" ><i class="fa fa-bolt" style="color:#fff"></i></a></p></td>															
										<td><p data-placement="top" data-toggle="tooltip" title="Pemeliharaan"><a data-target="#entryHar" class="btn btn-success btn-circle " data-title="Har" 
										data-toggle="modal" data-id="'.base64_encode(8888*$row["no"]).'" ><i class="fa fa-bolt" style="color:#fff"></i></a></p></td> 
										<td><p data-placement="top" data-toggle="tooltip" title="Gangguan Sistem"><a data-target="#entryGs" class="btn btn-info btn-circle " data-title="GSis" 
										data-toggle="modal" data-id="'.base64_encode(1111*$row["no"]).'" ><i class="fa fa-bolt" style="color:#fff"></i></a></p></td>															
									</tr>  
							'; $no++;	
							}
							echo 
							...
							<table class="table" id="dataFeed" width="100%" cellspacing="0">
								...
								echo '  
									<tr>
                        				<td>'.$no.' </td>																							 
										<td><p data-placement="top" data-toggle="tooltip" title="Lihat">
										<a data-target="#modViewFeeder" class="btn btn-warning btn-circle" data-title="Lihat" 
										data-toggle="modal" data-id="'.base64_encode(657454*$row["no"]).'">
										<i class="fa fa-eye" style="color:#fff"></i></a></p></td>							
										<td>'.$row["feeder"].'  '.$asal.'</td> 
										<td>'.$row["nmkltr"].'</td>  
										<td>'.$row["ulp"].' '.$spasi.''.$ulp2.'</td>
										<td>'.$row["scada"].'</td>  																
				...
				<div class="card-body">
					<div class="table-responsive">
                     <!-- Ini Isi Tabel-->			
						...
						{
							$query="SELECT*
							FROM pemadaman2 WHERE status <> 0 AND tanggalpadam BETWEEN DATE_SUB(NOW(), 
							INTERVAL 1 MONTH) AND NOW() order by no desc";
                        $result = mysqli_query($db, $query);
                        if (!$result) {
                          die('Invalid query: ' . mysqli_error());
                        }	
                        ...
                        <table class="table" id="dataFeed2" width="100%" cellspacing="0">
							...
							while ($row = @mysqli_fetch_assoc($result))
							{
								...
								else if ($row["jenispadam"] == 3)
								{$target = '#tleme';
								}
								else if ($row["jenispadam"] == 1)
								{$target = '#tlhar';								
								}
								else if ($row["jenispadam"] == 4)
								{$target = '#tlgsis';
								}
							echo '  
								<tr>
									<td>'.$no.' </td>
									<td><p data-placement="top" data-toggle="tooltip" title="Lanjut"><a data-target="'.$target.'" class="btn 
										';
										echo ''.$status.' ';
										echo 'btn-circle " data-title="Progress" data-toggle="modal" data-value="'.$row["status"].'" 
										data-id="'.base64_encode(1234*$row["no"]).'" >
										<i class="fa fa-pencil" style="color:#fff"></i></a></p></td>                        	
									<td>'.$row["feeder"].'  </td> 											
									<td>'.$row["tanggalpadam"].' '.$row["jampadam"].'</td>
									<td>'.$row["tanggalpulih"].' '.$row["jampulih"].' </td>
									<td>'.$row["lamapadam"].' '.$estimated.'</td> 							
									<td>'.$row["katapadam"].'  </td> 
									<td>'.$row["up3"].' '.$spasi1.''.$up32.' </td>  
									<td>'.$row["ulp"].' '.$spasi.''.$ulp2.'</td>
									<td>'.$row["indikasi"].' </td>
									<td>'.$row["tanggalnormal"].' '.$row["jamnormal"].' </td>																
									<td> <button id="'.$row["no"].'" class="delbutton btn btn-danger btn-circle"> 
									<i class="fa fa-trash" style="color:#fff"></i></button></td>
								</tr>  
                                '; $no++;	
                        	}
                            echo '
                        ...
                        while ($row = @mysqli_fetch_assoc($result))
							...
							echo '  
                                <tr>
                        			<td>'.$no.' </td>																							 
									<td><p data-placement="top" data-toggle="tooltip" title="Lihat">
									<a data-target="#modViewFeeder" class="btn btn-warning btn-circle" 
									data-title="Lihat" data-toggle="modal" data-id="'.base64_encode(657454*$row["no"]).'">
									<i class="fa fa-eye" style="color:#fff"></i></a></p></td>
        ...
        <div class="modal fade" id="modChangePw" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
		 ...
		 <div class="modal fade" id="tleme" role="dialog">
            <div class="modal-dialog modal-lg" role="document">
               <div class="modal-content">
                  <div class="tleme-data"> 
                  </div>
               </div>
            </div>
         </div>
		 <div class="modal fade" id="tlhar" role="dialog">
            <div class="modal-dialog modal-lg" role="document">
               <div class="modal-content">
                  <div class="tlhar-data"> 
                  </div>
               </div>
            </div>
         </div>
		 ...
		 <div class="modal fade" id="entryEme" role="dialog">
            <div class="modal-dialog" role="document">
               <div class="modal-content">
                  <div class="entryEme-data"></div>
               </div>
            </div>
         </div>
		 <div class="modal fade" id="entryHar" role="dialog">
            <div class="modal-dialog" role="document">
               <div class="modal-content">
                  <div class="entryHar-data"></div>
               </div>
            </div>
         </div>		 
