...
?>
	  ...
	  <div class="modal-header">
	          Lihat Hasil Ukur Tahanan Belitan - <?php echo $kodegd ; ?> <button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button> <input type="hidden" value="<?php echo $id; ?>" id="idgardu" disabled>
		</div>
...
<table width="760"  border="1" cellspacing="0" cellpadding="0">
  ...
  <?php
$kueri="SELECT*
FROM t3c WHERE kodegd ='$kodegd' AND status=1 ORDER by tgl DESC LIMIT 6" ;
$no=1;
...
if (!$hasil) {
 ...
 $id_t3c=190991*$kali*$brs["id_t3c"];  
  $kalik = base64_encode(13532*$kali);

 echo '  
  <tr>
    <td width="50"> <font color="#4f5051">&nbsp;&nbsp;'.$no.'</font> </td> 
    <td width="120">  <font color="#4f5051"><center><a href="#" onclick=JavaScript:view("../tier3/detail_c.php?id='.base64_encode(24021995*$kali*$id).'&assesment='.base64_encode($id_t3c).'&log='.$kalik.'")>'.$brs["kodegd"].'</a></center></font> </td>
